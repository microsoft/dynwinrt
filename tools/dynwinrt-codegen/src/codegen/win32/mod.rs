...
pub fn has_flat_functions(
    ...
    crate::win32_metadata::has_flat_functions(winmd_paths, namespace, class_name)
}
...
pub fn generate_namespace(
    ...
    let raw = crate::win32_metadata::parse_apis(winmd_paths, namespace, "Apis")
        .ok_or_else(|| format!("win32.no-complete-exports: no DLL exports in {namespace}"))?;
    let projection = project_apis(&raw);
    if projection.complete_count() == 0 {
        return Err(format!(
            "win32.no-complete-exports: {namespace}; {}",
            projection
                .omitted
                .iter()
                .map(|o| format!("{}: {}", o.identity, o.reason))
                .collect::<Vec<_>>()
                .join("; ")
        ));
    }
    ...
    Ok(GeneratedNamespace {
        ...
        js: output.js,
        dts: output.dts,
        extra_files: output.extra_files,
        exports: render::export_names(&projection.projected),
...
mod tests {
    ...
    fn fixed_buffer_projection_validates_minimum_storage() {
        ...
        assert!(output.js.contains("DynWin32.byteLength(buffer)"));
        assert!(!output.js.contains("buffer.byteLength"));
        assert!(!output.js.contains("value.byteLength"));
    }

    #[test]
    fn generated_headers_and_already_adjusted_imports_are_preserved() {
        let raw = RawApis {
            namespace: "Tests".into(),
            class_name: "Apis".into(),
            functions: vec![synthetic_function("Scalar")],
        };
        let import = "..\\..\\win32.js";
        let (output, omitted) = generate_apis_files(&raw, import);
        assert!(omitted.is_empty());
        assert!(output.js.starts_with("// Generated by dynwinrt-codegen"));
        assert!(output.dts.starts_with("// Generated by dynwinrt-codegen"));
        assert!(output.js.contains(&format!("require({import:?})")));
        assert!(
            output
                .js
                .contains(&format!("require({:?})", "..\\..\\win32-unsafe.js"))
        );
        assert!(output.dts.contains(&format!("from {import:?}")));
    }

    #[test]
    fn scalar_pointer_storage_uses_bounded_native_allocation() {
        let mut function = synthetic_function("ReadScalar");
        function.parameters.push(RawParameter {
            name: "value".into(),
            typ: RawType {
        ...
        assert!(output.js.contains("DynWin32.allocateBuffer(4)"));
        assert!(!output.js.contains("Buffer.alloc("));
        assert!(!output.js.contains(".subarray("));
    }
    ...
    fn native_struct_pointer_projects_typed_aligned_storage() {
        ...
        let (output, omitted) = generate_apis_files(&raw, "@microsoft/dynwinrt/win32");
        assert!(omitted.is_empty(), "{omitted:#?}");
        assert!(output.js.contains("_nativeAggregate.createNativeStruct"));
        assert!(output.js.contains("_nativeAggregate.nativeStruct(point"));
        assert!(
            output
                .dts
    ...
    fn zero_argument_native_struct_return_declares_its_layout() {
        ...
        assert!(
            output
                .js
                .contains("_nativeAggregate.toNativeStruct(_return, _nativeLayout_POINT)")
        );
    }
    ...
    fn direct_handle_ownership_uses_exact_function_evidence() {
        ...
        assert_eq!(
            omitted
                .iter()
                .filter(|o| o.reason == "win32.missing-raw-evidence")
                .count(),
            4
        );
        assert!(!output.js.contains("returnCleanup: \"localFree\""));
        ...
        let functions = cases
            .iter()
            .map(|(namespace, name, _, _, _)| {
                crate::win32_metadata::parse_apis(&winmd, namespace, "Apis")
                    .unwrap()
                    .functions
                    .into_iter()
                    .find(|f| f.name == *name)
