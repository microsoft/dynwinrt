...
pub(in crate::codegen::com) fn render_native_completion(
    plan: &super::super::ir::ProjectedNativeCompletion,
) -> Result<ComGeneratedOutput, String> {
    let descriptor = serde_json::to_string(plan).map_err(|error| error.to_string())?;
    let descriptor_literal =
        serde_json::to_string(&descriptor).map_err(|error| error.to_string())?;
    let import = com_runtime_import_name_for_module(plan.namespace);
    let import = serde_json::to_string(&import).map_err(|error| error.to_string())?;
    let function_name = plan.function_name;
    let js = format!(
        "// Generated by dynwinrt-codegen — do not edit\n\
         'use strict';\n\
         const {{ __activateAudioInterfaceAsync }} = require({import});\n\
         const descriptor = {descriptor_literal};\n\
         async function {function_name}(deviceInterfacePath, InterfaceClass) {{\n\
           if (arguments.length !== 2) throw new TypeError('Native audio activation requires exactly a device path and generated interface class; custom activation parameters are unsupported');\n\
           return __activateAudioInterfaceAsync(descriptor, deviceInterfacePath, InterfaceClass);\n\
         }}\n\
         exports.{function_name} = {function_name};\n"
    );
    let public_import = super::super::rebase_runtime_import(
        com_public_import_name(),
        super::super::canonical_namespace_depth(plan.namespace),
    );
    let dts = format!(
        "// Generated by dynwinrt-codegen — do not edit\n\
         import type {{ ComInterfaceType }} from '{public_import}';\n\
         /** Native one-shot activation on the caller's COM apartment. NULL activation parameters only.\n\
          * Targets: IAudioClient and IAudioEndpointVolume. May require UI-thread consent.\n\
          * Dropping the Promise does not cancel the Windows operation. */\n\
         export declare function {function_name}<T>(deviceInterfacePath: string, InterfaceClass: ComInterfaceType<T>): Promise<T>;\n"
    );
...
fn render_coclass_files(
    ...
    js.push_str(&format!(
        "import {{ DynCom, __registerComProjection }} from '{}';\n",
    ...
    js.push_str("}\n");
    js.push_str(&format!(
        "__registerComProjection({name}, {}.IID);\n",
        primary_interface.name
    ));
...
fn render_js(meta: &ProjectedComInterface) -> String {
    ...
    if meta
        .borrowed_storage
        .as_ref()
        .is_some_and(|storage| storage.copy_only)
    {
        out.push_str("// Bounded owned-copy facade; not a complete native interface projection.\n");
    }
    ...
    out.push_str(&format!(
        "import {{ {runtime_imports}, __registerComProjection, __comProjectionIid, projectAs }} from '{}';\n",
        com_runtime_import_name_for_module(&meta.namespace)
    ));
    if let Some(copy) = meta
        .borrowed_storage
        .as_ref()
        .and_then(|storage| storage.copy.as_ref())
    {
        let descriptor =
            serde_json::to_string(&copy.descriptor).expect("validated copy descriptor");
        out.push_str(&format!(
            "import {{ DynComBorrowedCopyPlan }} from '{}';\nlet _borrowedCopyPlan;\nconst _getBorrowedCopyPlan = () => _borrowedCopyPlan ??= DynComBorrowedCopyPlan.prepare({descriptor});\n",
            com_runtime_import_name_for_module(&meta.namespace)
        ));
    }
    for en in &meta.referenced_enums {
    ...
    let method_lines = meta
        ...
        .iter()
        .map(|method| {
            let mut signature = build_method_sig_js(method);
            if let Some(effect) = meta
                .borrowed_storage
                .as_ref()
                .and_then(|storage| storage.context_effects.get(&method.vtable_index))
            {
                signature.push_str(&format!(
                    ".withContextEffect({})",
                    serde_json::to_string(effect).expect("validated context descriptor")
                ));
            }
            format!(
                "            .addMethodAt({}, '{}', {})\n",
                method.vtable_index, method.name, signature
            )
        })
        .collect::<String>();
    ...
    if let Some(copy) = meta
        ...
        .and_then(|storage| storage.copy.as_ref())
    {
        use super::super::ir::CopyArguments;
        for operation in &copy.operations {
            let (parameters, arguments) = match operation.arguments {
                CopyArguments::None => ("", ""),
                CopyArguments::Bytes => ("data", ", data"),
                CopyArguments::Frames => ("frames", ", frames"),
                CopyArguments::Rectangle => ("rect", ", [rect.x, rect.y, rect.width, rect.height]"),
            };
            out.push_str(&format!("    {}({parameters}) {{\n        return _getBorrowedCopyPlan().{}(this._obj{arguments});\n    }}\n", operation.name, operation.runtime_method));
        }
    }
    ...
    for method in &meta.methods {
        ...
        match method.kind {
            ProjectedComMethodKind::RestrictedActivation { .. } => {
                emit_restricted_activation_js(&mut out, method, &iface_var)
            }
...
fn emit_restricted_activation_js(out: &mut String, method: &ProjectedComMethod, iface_var: &str) {
    let ProjectedComMethodKind::RestrictedActivation {
        iid_param_index,
        context_param_index,
        null_param_index,
        output_param_index,
        ...
        unreachable!("validated restricted activation")
    };
    let args = (0..method.params.len())
        .filter_map(|index| {
            if index == *iid_param_index {
                Some("DynCom.iidPointer(_iid)".to_string())
            } else if index == *context_param_index {
                Some(format!("DynCom.u32({context})"))
            } else if index == *null_param_index {
                Some("DynCom.exactNullPointer(null)".into())
            } else {
                assert_eq!(index, *output_param_index);
                None
            }
        })
        .collect::<Vec<_>>();
    let allowed = allowed_iids
        .iter()
        .map(|iid| format!("'{iid}'"))
        .collect::<Vec<_>>()
        .join(", ");
    out.push_str(&format!(
        "    {}(InterfaceClass) {{\n        const _iid = __comProjectionIid(InterfaceClass);\n        if (![{}].includes(_iid.toString().toLowerCase())) throw new TypeError('Interface is outside the supported null-parameter activation contract');\n        const _raw = {iface_var}.method({}).invoke(this._obj, [{}]);\n        let _owned;\n        try {{\n            _owned = DynCom.adoptComPointer(_raw, _iid);\n            return projectAs(_owned, InterfaceClass);\n        }} finally {{\n            if (_owned !== undefined) _owned.release();\n            _raw.release();\n        }}\n    }}\n",
        method.camel_name, allowed, method.vtable_index, args.join(", ")
    ));
}
...
fn render_dts(meta: &ProjectedComInterface) -> String {
    ...
    if meta
        .borrowed_storage
        .as_ref()
        .is_some_and(|storage| storage.copy.is_some())
    {
        out.push_str("import type { Buffer } from 'node:buffer';\n");
        if meta.borrowed_storage.as_ref().unwrap().copy_only {
            out.push_str(
                "/** Bounded owned-copy projection, not a complete native interface surface. */\n",
            );
        }
        out.push_str("/** Copy operations never expose native-backed views or caller-asserted lifetime. */\n");
    }
    ...
    if let Some(copy) = meta
        ...
        .and_then(|storage| storage.copy.as_ref())
    {
        use super::super::ir::{CopyArguments, CopyResult};
        for operation in &copy.operations {
            let parameters = match operation.arguments {
                CopyArguments::None => "",
                CopyArguments::Bytes => "data: Buffer | Uint8Array",
                CopyArguments::Frames => "frames: number",
                CopyArguments::Rectangle => {
                    "rect: { readonly x: number; readonly y: number; readonly width: number; readonly height: number }"
                }
            };
            let result = match operation.result {
                CopyResult::Void => "void",
                CopyResult::Bytes => "Buffer",
                CopyResult::Bitmap => "{ data: Buffer; width: number; height: number }",
                CopyResult::Packet => {
                    "({ kind: 'data' | 'silent'; data?: Buffer; frames: number; flags: number; timestampsValid: boolean; devicePosition?: bigint; qpcPosition?: bigint }) | null"
                }
            };
            out.push_str(&format!("    /** Synchronous owned-copy transaction; finalization failure throws and poisons this context. */\n    {}({parameters}): {result};\n", operation.name));
        }
    }
    let mut emitted_groups = std::collections::HashSet::new();
    for method in &meta.methods {
        ...
        if matches!(
            ...
            out.push_str(&format!(
                "    {}<T>(InterfaceClass: {{ readonly IID: WinGuid; _fromNative(value: unknown): T }}): T;\n",
                method.camel_name
            ));
            continue;
        }
        ...
        let (params, ret) = match method.kind {
            ProjectedComMethodKind::RestrictedActivation { .. } => unreachable!("emitted above"),
            ProjectedComMethodKind::Normal
            | ProjectedComMethodKind::FixedCapacityBytes { .. }
            | ProjectedComMethodKind::OwningCallerOutput { .. }
            | ProjectedComMethodKind::BorrowedStgMediumInput { .. }
