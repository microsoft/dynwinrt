...
fn project_instance_method(
    ...
    // only meant to be invoked by a host framework (in practice XAML). Projecting it as
    // a normal method either emits invalid syntax or exposes a misleading `ctor()` API.
    // The `.ctor` entry is still kept in interface registration so vtable indices stay correct.
