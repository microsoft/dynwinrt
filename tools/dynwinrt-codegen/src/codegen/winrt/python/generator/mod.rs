...
fn import_line(context: &PythonProjectionContext, needs_legacy_helper: bool) -> String {
    let object_input = context.support_symbol_import(PythonSupportSymbol::ObjectInput);
    let legacy_helper = if needs_legacy_helper {
...
{legacy_helper}    _dynwinrt_map, _dynwinrt_new_vector, _dynwinrt_ticks_to_datetime,
...
def _dynwinrt_legacy_call(impl, parameter_names, args, kwargs, public_name):
    bound = _dynwinrt_bind_overload(parameter_names, args, kwargs)
    if bound is None:
        raise TypeError(f'No matching overload for {public_name}')
    return impl(*bound)
\n";
