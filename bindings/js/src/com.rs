...
impl AutomationValue {
  ...
  pub(super) fn ensure_owner_thread(&self) -> napi::Result<()> {
...
struct ComInputGuard {
  ...
  bindings: Vec<super::ComApartmentBinding>,
}
...
impl ComInputGuard {
  fn pin(args: &[&DynWinRTValue]) -> napi::Result<Self> {
    ...
    Ok(Self {
      pins: args
        .iter()
        .filter_map(|arg| arg.7.as_ref())
        .map(super::com_input::InputBindings::pin)
        .collect::<napi::Result<Vec<_>>>()?,
      bindings: args
        .iter()
        .filter_map(|arg| arg.6.as_ref().cloned())
        .collect(),
    })
  }
...
#[cfg(test)]
thread_local! {
  static FORCE_NEXT_DELETE_OBJECT_FAILURE: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
}
...
impl DynComOwnedHandle {
  ...
  pub(super) fn cleanup_address(
    ...
    #[cfg(test)]
    let force_failure = FORCE_NEXT_DELETE_OBJECT_FAILURE.with(|fail| fail.replace(false));
    #[cfg(not(test))]
  ...
  #[cfg(test)]
  pub(super) fn force_next_delete_object_failure_for_test() {
    FORCE_NEXT_DELETE_OBJECT_FAILURE.with(|fail| fail.set(true));
  }
}
