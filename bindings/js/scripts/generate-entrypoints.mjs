...
const win32NativeExports = [
  ...
  'win32CallErrorCleanupFailures', 'win32CallErrorRetryCleanup',
...
const safeWin32Names = [
  'DynWin32', 'DynWin32CallError', 'DynWin32NativeStruct', 'DynWin32OverlappedOperation',
...
writeFileSync(join(distDir, 'win32-internal.js'), [
  ...
  'const RecoveryAggregateError = AggregateError',
  ...
  '  invoke(args) { return invokeNative(() => native.win32Invoke(this, args)) }',
  '  invokeWithSubsystem(context, subsystem, args) { return invokeNative(() => native.win32InvokeWithSubsystem(this, context, subsystem, args)) }',
  '}',
  ...
  '}',
  'class DynWin32CallError extends Error {',
  "  constructor() { throw new TypeError('Win32 call errors originate only from native dispatch') }",
  '  static getCleanupFailures(error) { return cleanupFailures(error) }',
  '  static retryCleanup(error) { return native.win32CallErrorRetryCleanup(error) }',
  '  get cleanupFailures() { return cleanupFailures(this) }',
  '  retryCleanup() { return native.win32CallErrorRetryCleanup(this) }',
  '}',
  ...
  '  return value',
  '}',
  'function recoveryProjectionError(value, error) {',
  '  try {',
  '    const aggregate = new RecoveryAggregateError([value, error], "Win32 cleanup error projection failed", { __proto__: null, cause: value })',
  '    defineProperties(aggregate, {',
  '      __proto__: null,',
  '      cleanupFailures: { __proto__: null, get: () => cleanupFailures(value) },',
  '      retryCleanup: { __proto__: null, value: () => native.win32CallErrorRetryCleanup(value) },',
  '    })',
  '    return aggregate',
  '  } catch {',
  '    // Never replace the owning carrier; static recovery does not require its projection.',
  '    return value',
  '  }',
  '}',
  'function cleanupFailures(value) {',
  '  const cached = recoveryRecords.get(value)',
  '  if (cached) return cached',
  '  try {',
  '    const records = native.win32CallErrorCleanupFailures(value).map((record) => Object.freeze({',
  '      target: Object.freeze(record.target),',
  '      error: Object.freeze(record.error),',
  '      resource: project(record.resource),',
  '    }))',
  '    Object.freeze(records)',
  '    recoveryRecords.set(value, records)',
  '    return records',
  '  } catch (error) {',
  '    if (native.win32CarrierKind(value) !== 8) throw error',
  '    throw recoveryProjectionError(value, error)',
  '  }',
  '}',
  'function invokeNative(call) {',
  '  let result',
  '  try { result = call() } catch (error) {',
  '    if (error === null || typeof error !== "object") throw error',
  '    try {',
  '      if (native.win32CarrierKind(error) === 8) project(error)',
  '    } catch (projectionError) {',
  '      throw recoveryProjectionError(error, projectionError)',
  '    }',
  '    throw error',
  '  }',
...
const carrierMembers = [
  ...
  [
    'static getCleanupFailures(error: unknown): readonly DynWin32CleanupFailure[]',
    'static retryCleanup(error: unknown): void',
    'readonly cleanupFailures: readonly DynWin32CleanupFailure[]',
    'retryCleanup(): void',
  ],
]
const win32CleanupDeclarations = [
  'export type DynWin32ResultTarget =',
  "  | { readonly kind: 'return' }",
  "  | { readonly kind: 'parameter'; readonly index: number }",
  "  | { readonly kind: 'aggregate-field'; readonly parameter: number; readonly field: number }",
  '/** Original cleanup diagnostics and a shared, managed owner; retained after successful retry. */',
  'export interface DynWin32CleanupFailure {',
  '  readonly target: DynWin32ResultTarget',
  '  /** code is the signed Windows HRESULT; message is the original cleanup error text. */',
  '  readonly error: { readonly code: number; readonly message: string }',
  '  readonly resource: DynWin32Resource',
  '}',
]
...
writeFileSync(join(distDir, 'index.d.ts'), [
  ...
  ...win32CleanupDeclarations,
  ...win32CarrierNames.flatMap((name, index) => [
    `export declare class ${name}${name === 'DynWin32CallError' ? ' extends Error' : ''} {`,
    '  private constructor()',
...
for (const [file, names, safe] of [
  ...
  writeFileSync(join(distDir, `${file}.d.ts`), [
    ...
    "export type { DynWin32CleanupFailure, DynWin32ResultTarget } from './index.js'",
    ...(safe ? [
