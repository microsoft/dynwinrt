...
and `/com/unsafe`, see [Classic COM JavaScript usage](classic-com-usage.md).
...
type; an `IEnum*` name is not ownership evidence. The exact registry also
covers `IEnumUnknown`, `IEnumVdsObject`, `IEnumEventObject`, and
`IEnumITfCompositionView`. Likewise, only NUL-terminated `*STR` aliases are
automatic strings. `PWCHAR`, `PCWCHAR`, `LPWCH`, `LPCWCH`, `LPCH`, and
...
[`tests/runners/com`](../tests/runners/com) and are generated
